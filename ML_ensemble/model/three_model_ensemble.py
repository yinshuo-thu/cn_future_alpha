...
def materialize_archived_outputs() -> None:
    ...
    previous = pd.DataFrame(report.get("previous_benchmarks", []))
    previous_cols = ["benchmark", "rows", "label_rows", "pooled_ic", "monthly_mean", "monthly_std", "monthly_ir"]
    if not previous.empty:
        previous = previous[[col for col in previous_cols if col in previous.columns]]
    previous.to_csv(ROOT / "metrics" / "previous_ensemble_benchmarks.csv", index=False)
...
def main() -> int:
    ...
    parser.add_argument("--materialize-archived", action="store_true", help="Write retained archived results to ML_ensemble.")
